...
public class CalcDestroyer {
	...
	private MobileElement resultFormula;
	...
	public double getResult(double result,int number, int operator) throws InterruptedException
	{
		if(operator==1)
		{			
			btnMultiply.click();
	...
	public double initCalc(int numberX , int numberY, int operator) throws InterruptedException
	{
		if(operator==1)
		{	
			getNumber(numberX);
	...
	public double getTxtResult() {
		...
		int intKodeMinus = 45;
		if(kode==8722) {
			char strBefore = new Character((char)kode);
			char strAfter = new Character((char)intKodeMinus);
			return Double.parseDouble(resultCalc.getText().replace(strBefore, strAfter));
		} else {
